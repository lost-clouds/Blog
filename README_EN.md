...
index.html (SPA)
  ...
  │   ├── #sec-dashboard    6 cards: device/CPU/memory/storage/uptime/battery
...
Without it, the battery card auto-hides without affecting other functionality.
...
| No root | `corn.sh` uses `top`/`free`/`uptime`/`getprop`/`ifconfig` (no `/proc`) |
